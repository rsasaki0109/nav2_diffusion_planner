...
**v0.9.0** — **両 seam での生成ファミリ拡充**リリース。v0.6.0–v0.8.0 で学習済みモデルをループに通しハイブリッドで完全性を担保したのに続き、v0.9.0 は**何が提案するか**を広げる: **transformer**(DETR 風 set-prediction・決定論的 single-forward)と **recurrent**(GRU 自己回帰ロールアウト・1 ウェイポイントずつ)の2つの生成ファミリを、**Mode A**(local trajectory・`OnnxTrajectoryModel`)と **Mode B**(global path・`OnnxPathModel`)の**両契約**に実装・出荷。Mode A=5系統(flow/diffusion/consistency/transformer/recurrent)、Mode B=3系統(flow/transformer/recurrent)。各ファミリは GPU 学習・CPU export した `model_zoo` アーティファクトを実 C++ 推論経路で走らせ、curated-zoo C++ テストで保護、ベンチ(`controller_benchmark`/`planner_benchmark`)と offline leaderboard(10構成)に掲載。正直な範囲: いずれも同一契約の**peer**で帰納バイアスを同一シナリオ比較するもの。transformer の独自性は**表現上**(提案が off-centre slot を向く — flow の CNN embedding には不可、A/B + C++ 方向テストで検証)だが、**狭い footprint 検証付き gap は依然未貫通**で、合成データ/容量の天井・hybrid 完全性保証は不変。実 ROS ホストで閉ループ sim 数値を取れる**discovery-free な Gazebo mission ハーネス**(`sim_mission.py`)も追加。基盤は v0.8.0(密結合 guided hybrid + DAgger)、v0.7.0(疎結合 hybrid・両モード全シナリオ)、v0.6.0(初の学習済みモデルを C++ ONNX で end-to-end・[model_zoo](model_zoo))、v0.5.0(ND + 比較ベンチ + 選択ガイド)、v0.4.0(**8 種 classical GlobalPlanner** + VFH+)、v0.3.0 までの生成系(決定論的安全層 + ONNX + flow/diffusion/consistency + costmap 条件付け + 学習パイプライン + RViz/Foxglove)。変更履歴は [CHANGELOG.md](CHANGELOG.md)、今後の計画は [docs/roadmap.md](docs/roadmap.md)。API は 1.0.0 まで安定化されていません。
