...
In addition, it experimentally collects **planners absent from upstream Nav2 — not only generative ones, but classical ones too.** As classical planners Nav2 does not ship, it implements, each as a `nav2_core::GlobalPlanner`: the **sampling-based** **RRT\*** (asymptotically optimal) and **RRT-Connect** (bidirectional, fast in narrow passages) in [nav2_rrt_planner](nav2_rrt_planner), **PRM** (Probabilistic Roadmap) in [nav2_prm_planner](nav2_prm_planner), the **incremental-search** **D\* Lite** (repairs only changed cells to cut replanning cost) in [nav2_dstar_lite_planner](nav2_dstar_lite_planner), the **grid-search speed-up** **JPS** (Jump Point Search, prunes symmetry to accelerate A\*) in [nav2_jps_planner](nav2_jps_planner), the **any-angle** **Lazy Theta\*** (straight-line paths not bound to grid directions, with lazy line-of-sight checks) in [nav2_lazy_theta_star_planner](nav2_lazy_theta_star_planner), the **anytime** **ARA\*** (progressively improves a bounded-suboptimal solution within a time budget) in [nav2_ara_star_planner](nav2_ara_star_planner), and the **geometric (continuous-space)** **visibility graph** (exact shortest straight-line path connecting obstacle convex corners) in [nav2_visibility_graph_planner](nav2_visibility_graph_planner). An offline table comparing these 8 on shared scenarios is in [docs/planner_comparison.md](docs/planner_comparison.md) (reproducible with `nav2_planner_benchmarks`). On the controller (local) side there are also 2 reactive avoidance controllers absent from Nav2: **VFH+** (Vector Field Histogram Plus, steers toward free valleys via a polar histogram) in [nav2_vfh_controller](nav2_vfh_controller) and **ND** (Nearness Diagram, gap selection + safety bias to keep to the corridor center) in [nav2_nd_controller](nav2_nd_controller), both as `nav2_core::Controller`.
...
**v0.9.0** — a **generative-family expansion across both seams** release. Following v0.6.0–v0.8.0, which put learned models in the loop and secured completeness via hybrids, v0.9.0 broadens **what proposes**: two generative families — **transformer** (DETR-style set-prediction, deterministic single-forward) and **recurrent** (GRU autoregressive rollout, one waypoint at a time) — are implemented and shipped on **both contracts**: **Mode A** (local trajectory, `OnnxTrajectoryModel`) and **Mode B** (global path, `OnnxPathModel`). Mode A = 5 families (flow/diffusion/consistency/transformer/recurrent), Mode B = 3 families (flow/transformer/recurrent). Each family runs its GPU-trained, CPU-exported `model_zoo` artifact through the real C++ inference path, is protected by curated-zoo C++ tests, and appears in the benchmarks (`controller_benchmark` / `planner_benchmark`) and the offline leaderboard (10 configs). Honest scope: all are **peers** on the same contract, comparing inductive biases on shared scenarios. The transformer's distinctiveness is **representational** (its proposals aim at the off-centre slot — impossible for flow's CNN embedding, verified by an A/B + a C++ direction test), but the **narrow footprint-validated gap was still not threaded** as of this release, and the synthetic-data / capacity ceiling and the hybrid completeness guarantee are unchanged. A **discovery-free Gazebo mission harness** (`sim_mission.py`) that can capture closed-loop sim numbers on a real ROS host was also added. The foundation is v0.8.0 (tightly-coupled guided hybrid + DAgger), v0.7.0 (loosely-coupled hybrid, all scenarios in both modes), v0.6.0 (the first learned models end-to-end in C++ ONNX, [model_zoo](model_zoo)), v0.5.0 (ND + comparison benchmarks + selection guide), v0.4.0 (**8 classical GlobalPlanners** + VFH+), and the generative stack through v0.3.0 (deterministic safety layer + ONNX + flow/diffusion/consistency + costmap conditioning + training pipeline + RViz/Foxglove). See [CHANGELOG.md](CHANGELOG.md) for history and [docs/roadmap.md](docs/roadmap.md) for plans. The API is not stabilized until 1.0.0.
