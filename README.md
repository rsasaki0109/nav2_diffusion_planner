...
<p align="center"><em>The shipped model <code>PathFlowPlanner</code> (flow matching) generates start→goal global path candidates, the deterministic costmap-validity layer rejects the ones that hit obstacles (red), and the shortest safe path (green) is selected — a propose→validate→select pipeline. As the obstacle moves left/right, the selected path switches to the opposite side. Reproduce with <a href="tools/mode_b_demo.py">tools/mode_b_demo.py</a>.</em></p>

<p align="center">
  <img src="docs/mcap_view.gif" width="420" alt="The mode_b_demo.mcap contents rendered in a Lichtblick/Foxglove-style view: costmap (red), candidate paths, selected path (green), goal">
</p>

<p align="center"><em>The same pipeline recorded to a portable <strong>MCAP</strong> (<a href="tools/foxglove_mcap_demo.py">tools/foxglove_mcap_demo.py</a> → <a href="docs/mode_b_demo.mcap">docs/mode_b_demo.mcap</a>) and opened in <strong>Lichtblick / Foxglove</strong> — no live ROS needed (web app: app.foxglove.dev → Open local file). Shown here is an honest render of the MCAP's actual messages (<code>/local_costmap</code>, <code>/path_best</code>, <code>/candidates_*</code>, <code>/goal_pose</code>) in a viewer-style 3D top-down (<a href="tools/mcap_view_gif.py">tools/mcap_view_gif.py</a>), not a screenshot; open the file yourself to scrub/play and Export → Video. See <a href="docs/visualization.md">docs/visualization.md</a>.</em></p>

This is the **Nav2 GlobalPlanner (Mode B)**, symmetric to the local controller (Mode A). No OSS integrating a generative model into `nav2_core::GlobalPlanner` existed at survey time, so it is implemented as a `PathModel` seam (analytic `FanPathModel` / learned `OnnxPathModel`, with costmap conditioning on the same seam) ([nav2_diffusion_global_planner](generative/nav2_diffusion_global_planner)). Like Mode A, learned Mode B ships **4 families — flow / transformer / recurrent (GRU autoregressive rollout) / attnseq (no-fan cross-attention autoregressive) —** in `model_zoo`. **The transformer aims proposals at an off-centre slot via token attention, and a footprint-aware (validator-aware) loss makes it the first Mode B model to thread the footprint-validated *off-centre gap* purely generatively** (6/8 courses). **The `attnseq` family goes all the way: it threads ALL eight benchmark courses purely generatively (8/8), including the two long documented as ceilings — the S-shaped *slalom* and the *far off-centre gap*.** Those turned out not to be an architecture limit but two data bugs (a training expert that grazed its own walls, and a train/inference patch-sampling mismatch); fixed with a collision-clean *plateau* expert and *deployment-matched* patches, a no-fan family — K learned seeds let a candidate take an *S* shape, which a uniform lateral fan cannot — clears them all (verified in the real C++ benchmark across 8 courses, no fallback). The same seam also carries **vehicle kinematics**: a kinematics-conditioned model reads the **min turn radius R** (`R=0` = omni) from a spare context input, so one model — the *same weights* — proposes a *sharp* detour for an omni / differential-drive robot and a *gentle* one for an Ackermann car through the same gap, and the planner's **curvature validator** disposes of any proposal that turns tighter than 1/R (propose/dispose extended from footprint to dynamics). Trained across all eight courses, the benchmark shows **omni 8/8** (threads everything incl. slalom, gate off), **diff 8/8** (sharp turning circle clears every lateral maneuver), and **Ackermann 3/8** (only the near-straight clear / centred gap / double gate — the curvature validator correctly rejects every course needing a lateral move past its 1.5 m turning circle). Cross-comparison is in [docs/planner_comparison.md](docs/planner_comparison.md); the (now-broken) ceilings and the honest research arc are in [docs/generative_limits.md](docs/generative_limits.md). The hybrid planner remains the completeness guarantee on arbitrary out-of-distribution maps.
...
<p align="center"><em>Closed-loop course assets in <a href="nav2_diffusion_sim">nav2_diffusion_sim</a> — <strong>real RViz2</strong> capture of grid A* routes on generated occupancy maps. Reproduce: <a href="tools/courses_mcap_demo.py">tools/courses_mcap_demo.py</a> → <a href="tools/record_rviz_gif.py">tools/record_rviz_gif.py</a>.</em></p>
...
| [docs/controller_comparison.md](docs/controller_comparison.md) | Closed-loop comparison of Mode A controllers — reactive (VFH+ / ND) and generative (learned / transformer / recurrent / **threading** / hybrid) (reach / clearance / steering / centering; auto-generated by `nav2_planner_benchmarks`). The **threading** model (costmap-token transformer, DAgger-trained on a corrected oracle, run with the windowed `safety_check_points` gate and a widened `costmap_patch_resolution` field of view) is the first learned Mode A controller here to thread **all** obstacle courses purely generatively — the dead-ahead *frontal* block, the *side obstacle*, **and the *corridor*** (where it centres better than the classical VFH+/ND baselines); the hybrid stays the completeness guarantee for novel scenes (docs/generative_limits.md). |
