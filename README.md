...
**v0.7.0** — **ハイブリッド: generative が提案し、classical が完全性を担保する**リリース。v0.6.0 で学習済みモデルをループに通し、小型合成モデルが gap-routing（Mode B）/ obstacle-threading（Mode A）で天井に当たることを正直に示した。v0.7.0 はその天井を**小型モデルへの過学習でなく設計で**突破: 各モードに classical fallback を追加し、learned が提案（簡単な地図は高速・多峰・costmap 条件付き）、無効なら**完全な classical 手法が引き継ぐ**。Mode B planner は classical **探索**（JPS）へ、Mode A controller は classical **reactive**（VFH+）へ fallback し、**両モードとも全ベンチシナリオで goal 到達 / 有効パス返却**。効きどころ・天井・超え方は [docs/generative_limits.md](docs/generative_limits.md)。基盤は v0.6.0（初の学習済み Mode A/B モデルを C++ ONNX 推論で end-to-end・[model_zoo](model_zoo) 実体化）、v0.5.0（ND controller + 比較ベンチ 2 種 + 選択ガイド）、v0.4.0（**8 種 classical GlobalPlanner** + VFH+）、v0.3.0 までの生成系（決定論的安全層 + ONNX + flow/diffusion/consistency + costmap 条件付け + 学習パイプライン + RViz/Foxglove）。変更履歴は [CHANGELOG.md](CHANGELOG.md)、今後の計画は [docs/roadmap.md](docs/roadmap.md)。API は 1.0.0 まで安定化されていません。
