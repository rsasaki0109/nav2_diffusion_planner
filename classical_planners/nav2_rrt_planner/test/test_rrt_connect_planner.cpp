...
// Integration test for the RRTConnectPlanner: drives the real plugin through
// configure/activate against a live nav2_costmap_2d::Costmap2DROS (no Gazebo /
// GPU). It returns a path on a clear map, threads an off-centre gap in a wall
// (the bidirectional connect step's strength), connects through a narrow gap
// in far fewer iterations than plain RRT would, and fails cleanly when the
// corridor is fully blocked or the goal is occupied.
...
class RRTConnectPlannerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("rrt_connect_planner_test");
    ...
    planner_ = std::make_shared<nav2_rrt_planner::RRTConnectPlanner>();
  ...
  static std::shared_ptr<nav2_rrt_planner::RRTConnectPlanner> planner_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> RRTConnectPlannerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> RRTConnectPlannerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> RRTConnectPlannerTest::tf_;
std::shared_ptr<nav2_rrt_planner::RRTConnectPlanner> RRTConnectPlannerTest::planner_;
...
TEST_F(RRTConnectPlannerTest, ClearMapReturnsCollisionFreePathToGoal)
{
  clearCostmap();
...
TEST_F(RRTConnectPlannerTest, RoutesThroughOffCentreGap)
{
  clearCostmap();
...
TEST_F(RRTConnectPlannerTest, SolidWallThrowsNoValidPath)
{
  clearCostmap();
...
TEST_F(RRTConnectPlannerTest, GoalInLethalCellThrowsGoalOccupied)
{
  clearCostmap();
...
TEST_F(RRTConnectPlannerTest, CancelDuringPlanningThrows)
{
  clearCostmap();
  EXPECT_THROW(
