...
// Integration test for the PRMPlanner: drives the real plugin through
...
// wall via the sampled roadmap, and fails cleanly when the corridor is fully
...
class PRMPlannerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("prm_planner_test");
    ...
    planner_ = std::make_shared<nav2_prm_planner::PRMPlanner>();
  ...
  static std::shared_ptr<nav2_prm_planner::PRMPlanner> planner_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> PRMPlannerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> PRMPlannerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> PRMPlannerTest::tf_;
std::shared_ptr<nav2_prm_planner::PRMPlanner> PRMPlannerTest::planner_;
...
TEST_F(PRMPlannerTest, ClearMapReturnsCollisionFreePathToGoal)
{
  clearCostmap();
...
TEST_F(PRMPlannerTest, RoutesThroughOffCentreGap)
{
  clearCostmap();
...
TEST_F(PRMPlannerTest, SolidWallThrowsNoValidPath)
{
  clearCostmap();
...
TEST_F(PRMPlannerTest, GoalInLethalCellThrowsGoalOccupied)
{
  clearCostmap();
...
TEST_F(PRMPlannerTest, CancelDuringPlanningThrows)
{
  clearCostmap();
  EXPECT_THROW(
