...
// Integration test for the DStarLitePlanner: drives the real plugin through
// configure/activate against a live nav2_costmap_2d::Costmap2DROS (no Gazebo /
// GPU). Beyond the usual clear-map / off-centre-gap / blocked-wall / occupied-
// goal / cancel cases, it exercises D* Lite's defining feature: an INCREMENTAL
// replan that reuses cached search state — plan on a clear map, then drop a wall
// in and replan with the same goal, expecting the repaired path to avoid it.
...
class DStarLitePlannerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("dstar_lite_planner_test");
    ...
    planner_ = std::make_shared<nav2_dstar_lite_planner::DStarLitePlanner>();
  ...
  static std::shared_ptr<nav2_dstar_lite_planner::DStarLitePlanner> planner_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> DStarLitePlannerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> DStarLitePlannerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> DStarLitePlannerTest::tf_;
std::shared_ptr<nav2_dstar_lite_planner::DStarLitePlanner> DStarLitePlannerTest::planner_;
...
TEST_F(DStarLitePlannerTest, ClearMapReturnsCollisionFreePathToGoal)
{
  clearCostmap();
...
TEST_F(DStarLitePlannerTest, RoutesThroughOffCentreGap)
{
  clearCostmap();
...
TEST_F(DStarLitePlannerTest, IncrementalReplanAvoidsNewObstacle)
{
  // First plan on a clear map (cold start), then drop a wall in and replan with
  // the SAME goal so D* Lite reuses and repairs its cached search rather than
  // starting over. The repaired path must avoid the new wall.
  clearCostmap();
  const auto first = planner_->createPlan(pose(1.0, 3.0), pose(5.0, 3.0), noCancel);
  ASSERT_TRUE(planIsCollisionFree(first));

  markWall(3.0, /*gap_center=*/1.0, /*gap_half=*/0.4);  // wall appears mid-corridor
  const auto repaired = planner_->createPlan(pose(1.0, 3.0), pose(5.0, 3.0), noCancel);
  ASSERT_GE(repaired.poses.size(), 2u);
  EXPECT_TRUE(planIsCollisionFree(repaired));
  double min_y = 1e9;
  for (const auto & p : repaired.poses) {
    min_y = std::min(min_y, p.pose.position.y);
  }
  EXPECT_LT(min_y, 1.6);  // detours toward the gap after the incremental repair
}

TEST_F(DStarLitePlannerTest, SolidWallThrowsNoValidPath)
{
  clearCostmap();
...
TEST_F(DStarLitePlannerTest, GoalInLethalCellThrowsGoalOccupied)
{
  clearCostmap();
...
TEST_F(DStarLitePlannerTest, CancelDuringPlanningThrows)
{
  clearCostmap();
  EXPECT_THROW(
