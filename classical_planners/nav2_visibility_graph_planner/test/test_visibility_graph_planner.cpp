...
// Integration test for the VisibilityGraphPlanner: drives the real plugin
// through configure/activate against a live nav2_costmap_2d::Costmap2DROS (no
// Gazebo / GPU). On a clear map it returns a straight line to an off-axis goal
// (the visibility edge start->goal), routes around a wall through the gap, and
// fails cleanly when blocked or the goal is occupied.
...
class VisibilityGraphPlannerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("visibility_graph_planner_test");
    ...
    planner_ = std::make_shared<nav2_visibility_graph_planner::VisibilityGraphPlanner>();
  ...
  static std::shared_ptr<nav2_visibility_graph_planner::VisibilityGraphPlanner> planner_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> VisibilityGraphPlannerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> VisibilityGraphPlannerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> VisibilityGraphPlannerTest::tf_;
std::shared_ptr<nav2_visibility_graph_planner::VisibilityGraphPlanner>
VisibilityGraphPlannerTest::planner_;
...
TEST_F(VisibilityGraphPlannerTest, ClearMapReturnsStraightLineToGoal)
{
  clearCostmap();
  ...
  EXPECT_NEAR(plan.poses.front().pose.position.y, 1.0, 1e-6);
  ...
  // With no obstacles the only edge needed is start->goal: a straight line.
  EXPECT_LT(maxDeviationFromLine(plan, 1.0, 1.0, 5.0, 3.0), 0.05);
}

TEST_F(VisibilityGraphPlannerTest, RoutesThroughOffCentreGap)
{
  clearCostmap();
  markWall(3.0, /*gap_center=*/1.0, /*gap_half=*/0.5);  // gap off the straight line
...
TEST_F(VisibilityGraphPlannerTest, SolidWallThrowsNoValidPath)
{
  clearCostmap();
...
TEST_F(VisibilityGraphPlannerTest, GoalInLethalCellThrowsGoalOccupied)
{
  clearCostmap();
...
TEST_F(VisibilityGraphPlannerTest, CancelDuringPlanningThrows)
{
  clearCostmap();
  EXPECT_THROW(
