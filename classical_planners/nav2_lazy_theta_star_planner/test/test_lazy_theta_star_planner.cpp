...
// Integration test for the LazyThetaStarPlanner: drives the real plugin through
...
// goal / cancel cases, it checks Lazy Theta*'s defining property: on a clear map
// the path to an off-axis goal is a single straight (any-angle) line, not a
// staircase of 8-connected grid moves.
...
class LazyThetaStarPlannerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("lazy_theta_star_planner_test");
    ...
    planner_ = std::make_shared<nav2_lazy_theta_star_planner::LazyThetaStarPlanner>();
  ...
  // Max perpendicular distance of any path pose from the straight start->goal line.
  static double maxDeviationFromLine(
    const nav_msgs::msg::Path & plan, double x0, double y0, double x1, double y1)
  {
    const double len = std::hypot(x1 - x0, y1 - y0);
    double worst = 0.0;
    for (const auto & p : plan.poses) {
      const double cross = std::abs(
        (x1 - x0) * (y0 - p.pose.position.y) - (x0 - p.pose.position.x) * (y1 - y0));
  ...
  static std::shared_ptr<nav2_lazy_theta_star_planner::LazyThetaStarPlanner> planner_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> LazyThetaStarPlannerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> LazyThetaStarPlannerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> LazyThetaStarPlannerTest::tf_;
std::shared_ptr<nav2_lazy_theta_star_planner::LazyThetaStarPlanner>
LazyThetaStarPlannerTest::planner_;
...
TEST_F(LazyThetaStarPlannerTest, ClearMapReturnsCollisionFreePathToGoal)
{
  clearCostmap();
...
TEST_F(LazyThetaStarPlannerTest, AnyAngleStraightLineOnClearMap)
{
  // Off-axis goal: an 8-connected grid planner would staircase; Lazy Theta* must
  // return a single straight any-angle line, so deviation from it is ~0.
  clearCostmap();
  const auto plan = planner_->createPlan(pose(1.0, 1.0), pose(5.0, 3.0), noCancel);
  ASSERT_GE(plan.poses.size(), 2u);
  EXPECT_TRUE(planIsCollisionFree(plan));
  EXPECT_LT(maxDeviationFromLine(plan, 1.0, 1.0, 5.0, 3.0), 0.05);
}

TEST_F(LazyThetaStarPlannerTest, RoutesThroughOffCentreGap)
{
  clearCostmap();
...
TEST_F(LazyThetaStarPlannerTest, SolidWallThrowsNoValidPath)
{
  clearCostmap();
...
TEST_F(LazyThetaStarPlannerTest, GoalInLethalCellThrowsGoalOccupied)
{
  clearCostmap();
...
TEST_F(LazyThetaStarPlannerTest, CancelDuringPlanningThrows)
{
  clearCostmap();
  EXPECT_THROW(
