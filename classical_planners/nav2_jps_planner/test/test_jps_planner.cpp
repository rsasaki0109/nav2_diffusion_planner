...
// Integration test for the JPSPlanner: drives the real plugin through
// configure/activate against a live nav2_costmap_2d::Costmap2DROS (no Gazebo /
// GPU). It returns an optimal collision-free path on a clear map, routes through
// an off-centre gap in a wall, and fails cleanly when the corridor is fully
...
class JPSPlannerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("jps_planner_test");
    ...
    planner_ = std::make_shared<nav2_jps_planner::JPSPlanner>();
  ...
  static std::shared_ptr<nav2_jps_planner::JPSPlanner> planner_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> JPSPlannerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> JPSPlannerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> JPSPlannerTest::tf_;
std::shared_ptr<nav2_jps_planner::JPSPlanner> JPSPlannerTest::planner_;
...
TEST_F(JPSPlannerTest, ClearMapReturnsCollisionFreePathToGoal)
{
  clearCostmap();
...
TEST_F(JPSPlannerTest, RoutesThroughOffCentreGap)
{
  clearCostmap();
...
TEST_F(JPSPlannerTest, SolidWallThrowsNoValidPath)
{
  clearCostmap();
...
TEST_F(JPSPlannerTest, GoalInLethalCellThrowsGoalOccupied)
{
  clearCostmap();
...
TEST_F(JPSPlannerTest, CancelDuringPlanningThrows)
{
  clearCostmap();
  EXPECT_THROW(
