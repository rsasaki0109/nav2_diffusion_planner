...
> **Gazebo bring-up 状況（2026-06-05 実走検証）**: `tb3_gazebo_diffusion.launch.py` を
> headless 起動し、**gz の GPU LiDAR 描画（/scan count 360）・ros_gz bridge・Nav2 ロード・
> DiffusionController 設定までは実走で確認済み**。完走をブロックしたのは (1) 初期姿勢未設定
> で localization が activate しない、(2) このサンドボックスの DDS が外部プロセスを受け付け
> ない（Fast DDS=`/dev/shm` ロック失敗で外部 CLI が join 不可、CycloneDDS=participant 生成
> 失敗）こと。よって**自動の閉ループ数値ベンチは本環境では完走不可**。詳細と再開手順は
> [simulation.md](simulation.md) §10.5。実装すべきは「初期姿勢 publish → `NavigateToPose`
> 送信 → `/odom` 集計→結果書き出し」を **launch 内に内包する mission ノード**（外部 discovery
> 非依存）＋ DDS の通る実 ROS ホスト。
