...
> **Gazebo bring-up + mission ハーネス（2026-06-05 実走検証・実装済み）**:
> `tb3_gazebo_diffusion.launch.py` を headless 起動し、**gz の GPU LiDAR 描画
> （/scan count 360）・ros_gz bridge・Nav2 ロード・DiffusionController 設定を実走で確認**。
> 完走のための **mission ハーネスを実装済み**: amcl `set_initial_pose`（localization 自前
> 起動）＋ in-launch `sim_mission.py`（goal 誘導→/odom 集計→**結果を md ファイルへ**）＋
> `tb3_gazebo_mission.launch.py`（sim＋mission を 1 launch、mission 終了で Shutdown）。
> mission は実走して結果ファイルを正しく書き出した＝**ハーネスは健全**。唯一残る壁は
> **このサンドボックスの inter-process DDS が完全に不通**（talker/listener 2 プロセスでも
> 受信 0。Fast DDS の SHM/UDP/unicast-XML 全滅、Cyclone は init 不可、discovery-server
> 未導入）であること。nav2 は単一コンテナ内通信で起動するが、別プロセスの mission が
> discover できず `action server unavailable`。**実 ROS ホスト**（DDS が通る環境）へ移せば
> 上記 mission launch がそのまま実 sim numbers を出す。詳細・全証拠は
> [simulation.md](simulation.md) §10.5。
