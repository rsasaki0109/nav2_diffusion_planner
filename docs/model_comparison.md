...
8つの生成プランナ構成（flow / diffusion / consistency / transformer × context / costmap+goal）を**同一の合成回避シナリオ**で評価し、各モデルが提案する K 候補軌道の品質を比較する。シナリオは前方を塞ぐ障害物と片側 gap（gap-right / gap-left）＋ clear。直進すると衝突し、gap 側へ回避した候補のみクリアできる。これは CPU・決定論的な**提案品質**の比較であり、シミュレータ走行ベンチ（`benchmark_runner`）とは別物。
...
|---|---|---|---|--:|--:|--:|--:|--:|--:|--:|
| 1 | `costmap-consistency` | consistency (1-step) | costmap+goal | 1 | 1.00 | 0.543 | 0.564 | 0.096 | 0.00 | **0.883** |
| 2 | `costmap-transformer` | transformer (set-pred) | costmap+goal | 1 | 1.00 | 0.544 | 0.529 | 0.126 | 0.00 | **0.872** |
| 3 | `costmap-flow` | flow matching | costmap+goal | 2 | 0.67 | 0.537 | 0.588 | 0.593 | 0.56 | **0.711** |
| 4 | `transformer` | transformer (set-pred) | context | 1 | 0.33 | 0.513 | 0.859 | 0.010 | 0.67 | **0.632** |
| 5 | `consistency` | consistency (1-step) | context | 1 | 0.33 | 0.512 | 0.838 | 0.059 | 0.67 | **0.619** |
| 6 | `costmap-diffusion` | diffusion (DDIM) | costmap+goal | 4 | 0.33 | 0.509 | 0.612 | 0.935 | 0.67 | **0.458** |
| 7 | `diffusion` | diffusion (DDIM) | context | 4 | 0.33 | 0.525 | 0.294 | 0.688 | 0.67 | **0.429** |
| 8 | `flow` | flow matching | context | 1 | 0.67 | 0.525 | 0.027 | 2.660 | 0.56 | **0.302** |
...
- **costmap-consistency**（1ステップ蒸留）が衝突0・success 1.00 で総合首位、**costmap-transformer**（DETR 風 set-prediction）が衝突0・success 1.00・1ステップで僅差の2位。新規の transformer ファミリが反復サンプリング系と同等の安全候補品質を1フォワードで出せることを示す（context-only でもtransformer が context 勢の最上位）。
