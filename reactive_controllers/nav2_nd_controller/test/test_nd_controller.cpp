...
// Closed-loop integration test for the NDController: drives the real plugin
// through configure/activate against a live nav2_costmap_2d::Costmap2DROS (no
// Gazebo / GPU). Checks it drives straight on a clear path, steers around a
// frontal obstacle, deflects away from a close obstacle on one side (the ND
// safety-deflection hallmark), stops without a plan, and stops at the goal.
...
class NDControllerTest : public ::testing::Test
{
protected:
  static void SetUpTestSuite()
  {
    ...
    node_ = std::make_shared<rclcpp_lifecycle::LifecycleNode>("nd_controller_test");
    ...
    controller_ = std::make_shared<nav2_nd_controller::NDController>();
  ...
  static std::shared_ptr<nav2_nd_controller::NDController> controller_;
};

std::shared_ptr<rclcpp_lifecycle::LifecycleNode> NDControllerTest::node_;
std::shared_ptr<nav2_costmap_2d::Costmap2DROS> NDControllerTest::costmap_ros_;
std::shared_ptr<tf2_ros::Buffer> NDControllerTest::tf_;
std::shared_ptr<nav2_nd_controller::NDController> NDControllerTest::controller_;

TEST_F(NDControllerTest, DrivesForwardOnClearPath)
{
  clearCostmap();
...
TEST_F(NDControllerTest, SteersAroundFrontalObstacle)
{
  clearCostmap();
  markLethalAt(kRobotX + 0.35, kRobotY, 2);  // obstacle ahead within security distance
  controller_->setPlan(straightPlan());
  const auto cmd = controller_->computeVelocityCommands(
    robotPose(), geometry_msgs::msg::Twist(), nullptr);
  EXPECT_GT(std::abs(cmd.twist.angular.z), 0.05);
  EXPECT_GE(cmd.twist.linear.x, 0.0);
}

TEST_F(NDControllerTest, DeflectsAwayFromCloseRightObstacle)
{
  // ND safety-deflection hallmark: a close obstacle on the right (no obstacle
  // ahead) steers the robot left, away from it.
  clearCostmap();
  markLethalAt(kRobotX, kRobotY - 0.3, 1);  // ~0.3 m to the robot's right
  controller_->setPlan(straightPlan());
  const auto cmd = controller_->computeVelocityCommands(
    robotPose(), geometry_msgs::msg::Twist(), nullptr);
  EXPECT_GT(cmd.twist.angular.z, 0.0);  // turns left, away from the right obstacle
  EXPECT_GT(cmd.twist.linear.x, 0.0);   // path ahead is clear, keeps moving
}

TEST_F(NDControllerTest, StopsWhenNoPlan)
{
  clearCostmap();
...
TEST_F(NDControllerTest, StopsAtGoal)
{
  clearCostmap();
  nav_msgs::msg::Path path;
  path.header.frame_id = "map";
  ...
  p.pose.position.x = kRobotX + 0.05;
