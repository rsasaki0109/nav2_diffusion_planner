...
加えて **生成型 Mode B**（[nav2_diffusion_global_planner](../nav2_diffusion_global_planner)）も同じ表に並べる。デフォルトの解析的 `FanPathModel`（ONNX 不要・決定論的）で動き、「モデルが候補を *propose* → 決定論的安全層が *dispose*」を同一土俵で示す。
...
- **生成型 Mode B** は探索系と違い**完全ではない**。clear / off-centre gap は成功するが、slalom（S 字）では解析的 fan の提案分布が形状を表現できず、安全層が全候補を棄却して **no path**（安全側に fail-closed）になる。学習済みモデルならデータ駆動でより豊かな形状を提案でき、同じ安全層がそれを検証する。これが「propose / dispose」分業の意図した挙動。
