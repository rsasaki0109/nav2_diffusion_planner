...
int main(int argc, char ** argv)
{
  ...
  // The recurrent Mode A sibling (GRU autoregressive rollout) shares the exact same
  // ONNX contract too, so it is likewise a drop-in for the same controller.
  const std::string recurrent_model =
    ament_index_cpp::get_package_share_directory("nav2_planner_benchmarks") +
    "/models/diffusion_local_costmap_recurrent.onnx";
  // The obstacle-threading Mode A model: the costmap-token transformer DAgger-trained on
  // a corrected reactive dodge oracle, run with the windowed footprint gate
  // (safety_check_points). Unlike the other learned Mode A models it threads obstacles in
  // closed loop instead of stalling (docs/generative_limits.md).
  const std::string threading_model =
    ament_index_cpp::get_package_share_directory("nav2_planner_benchmarks") +
    "/models/diffusion_local_costmap_threading.onnx";
  ...
  const std::vector<ControllerEntry> controllers = {
    ...
    {"Diffusion (Mode A, transformer)", "nav2_diffusion_controller::DiffusionController",
      {rclcpp::Parameter("model_plugin", std::string("nav2_diffusion_onnx::OnnxTrajectoryModel")),
        ...
        rclcpp::Parameter("lookahead_distance", 1.0),
        rclcpp::Parameter("max_linear_speed", 1.5)}},
    {"Diffusion (Mode A, recurrent)", "nav2_diffusion_controller::DiffusionController",
      {rclcpp::Parameter("model_plugin", std::string("nav2_diffusion_onnx::OnnxTrajectoryModel")),
        rclcpp::Parameter("model_path", recurrent_model),
        rclcpp::Parameter("costmap_patch_size", 32),
        // Same conditioning as the flow Mode A model (carrot ~1 m ahead, trained
        // max_angular 1.0); only the proposer architecture differs (GRU rollout).
        rclcpp::Parameter("lookahead_distance", 1.0),
        rclcpp::Parameter("max_linear_speed", 1.5)}},
    {"Diffusion (Mode A, threading)", "nav2_diffusion_controller::DiffusionController",
      {rclcpp::Parameter("model_plugin", std::string("nav2_diffusion_onnx::OnnxTrajectoryModel")),
        ...
        // the robot actually executes before re-planning, so a tight reactive skirt is
        // not hard-rejected for clipping the block with its far lookahead. This + the
        // corrected-oracle DAgger model is what lets learned Mode A thread obstacles.
  ...
  std::cout << "# Local controller comparison "
    "(VFH+ / ND / learned, transformer, recurrent, threading & hybrid Mode A)\n\n";
  ...
  std::cout << "All are `nav2_core::Controller` plugins absent from upstream Nav2 "
    ...
    "set-prediction, identical ONNX contract) — a same-contract drop-in proposer. "
    "**Diffusion (Mode A, recurrent)** is another such drop-in: the GRU "
    "autoregressive-rollout model_zoo sibling that emits the trajectory one point at "
    "a time (same contract, sequential inductive bias). **Diffusion (Mode A, "
    "threading)** is the first learned Mode A controller here to **thread an obstacle "
    "purely generatively** (no fallback): it is the costmap-token transformer "
    "DAgger-trained on a *corrected* reactive dodge oracle (the shipped oracle collided; "
  ...
  std::cout << "_Min clearance is the smallest robot-to-obstacle distance over the "
    ...
    "collision). The **transformer** and **recurrent** Mode A models (same ONNX "
    "contract, different proposer architectures — set-prediction and GRU rollout) "
    "behave equivalently — reach *open*, safe-stop on the obstacle scenarios — "
    "confirming the ceiling is the synthetic data / capacity, not the model family. "
    "The **hybrid** adds a classical fallback (VFH+) that takes "
    "over when "
